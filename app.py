...
def home():
    ...
    if "email" in session:
        ...
        return render_template("welcome_logged_in.html", login_user=login_user, tenders_id=tenders_id)

    return render_template("welcome.html", tenders_id=tenders_id)
...
def results(tenderId):
    ...
    if "email" in session:
        ...
        if ((current_tender["results"] and email in current_tender["participants"])
            or (email == current_tender["owner"])):
            ...
            items_list_id2 = list(current_tender["results"].values()) # list of the results items id
            for i in range(size):
